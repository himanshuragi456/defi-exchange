...
class MarketDepthContainer extends React.Component {
    constructor() {
        ...
        this.convertToCumulative = (data, type) => {
            ...
            const tipLayout = ({
                ...
                <span className="parent-market-depth__tooltip">
                    <span>
            ...
            const cumulative = data.map((item, index) => {
                ...
                return {
                    ...
                    cumulativePrice: preciseData(
                        cumulativePriceData,
                        this.props.currentMarket.price_precision
                    ),
                    cumulativeVolume: preciseData(
    ...
    shouldComponentUpdate(prev, next) {
        ...
        const ordersLength =
            Number(asksItems.length) + Number(bidsItems.length);
        return (
            ordersLength !==
                this.props.asksItems.length + this.props.bidsItems.length ||
            prev.colorTheme !== this.props.colorTheme ||
            asksItems !== this.props.asksItems ||
            bidsItems !== this.props.bidsItems
        );
    }
        ...
        return (
            <MarketDepths
                settings={{
                    ...settings,
                    ...{
                        height: MarketDepths ? MarketDepths.clientHeight : 200,
                    },
                }}
                className="parent-market-depth"
                colors={colors}
                data={
                    this.props.asksItems.length > 0 &&
                    this.props.bidsItems.length > 0
                        ? this.convertToDepthFormat()
                        : null
                }
                ref="MarketDepths"
            />
    ...
    convertToDepthFormat() {
        ...
        const resultLength =
            asksItemsLength > bidsItemsLength
                ? bidsItemsLength
                : asksItemsLength;
