...
class OrderBookContainer extends React.Component {
    constructor() {
        ...
        this.orderBook = (bids, asks) => (
            <OrderBook
                ...
                title={this.props.intl.formatMessage({
                    id: "page.body.trade.header.bids",
                })}
                headers={this.renderHeaders()}
                data={this.renderOrderBook(
                    bids,
                    "bids",
            ...
            return array.length > 0
                ? array.map((item, i) => {
                      ...
                      return [
                          ...
                          <span
                              style={{
                                  color: "var(--open-orders-order-buy)",
                              }}
                              key={i}
                          >
                              <Decimal fixed={priceFixed}>{price}</Decimal>
                          </span>,
                      ];
                  })
                : [[[""], message]];
        ...
        this.handleOnSelect = (index) => {
            ...
            const priceToSet = bids[Number(index)]
                ? bids[Number(index)][0]
                : "";
    ...
    render() {
        ...
        const cn = classNames("", {
            "parent-bids--loading": bidsLoading,
        });
        return (
            <div className={cn}>
                {bidsLoading ? (
                    ...
                    this.orderBook(sortBids(bids), asks)
                )}
            </div>
        );
    }
