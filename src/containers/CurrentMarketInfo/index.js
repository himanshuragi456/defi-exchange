...
class CurrentMarketInfoContainer extends React.Component {
    ...
    render() {
        ...
        const currentCurrency =
            currentMarket &&
            currencies.filter(
                (currency) => currency.id === currentMarket.base_unit
            );
        let iconUrl = "";
        ...
        const currentMarketTicker =
            marketTickers[currentMarket && currentMarket.id];
        const isPositive =
            currentMarket &&
            /\+/.test(
        ...
        const currentQuoteUnit =
            currentMarket &&
            currencies.filter(
                (currency) => currency.id === currentMarket.quote_unit
            )[0];
        const mobilePriceClassName = classnames(
            "current-market-info__change mt-1",
            {
        ...
        return (
            <div className="d-flex justify-content-between align-items-center px-3 py-1 current-market-info-container">
                <div className="d-flex align-items-center">
                    <div>
                        <img
                            src={iconUrl || DefaultCurrencyIcon}
                            alt={"c-market"}
                            style={{
                                width: "25px",
                            }}
                    ...
                    {currentMarket && (
                        <div className="ml-2 text-bright">
                ...
                <div className="d-flex">
                    <div className="d-flex flex-column mr-2 align-items-end">
                        <h6 className="font-bold mb-0 text-bright">
                            {currentMarketTicker &&
                                Number(currentMarketTicker.last).toFixed(
                        ...
                        <p>
                            <span className="text-muted-2">
                                ...
                                {currentMarketTicker &&
                                    Number(currentMarketTicker.vol).toFixed(
                                        currentMarket.amount_precision
                                    )}
                            </span>
                        </p>
                    </div>
                    {currentMarket && currentMarketTicker && (
                        <div className={`${mobilePriceClassName} px-2`}>
                            <span>
                                {currentMarketTicker &&
...
export const CurrentMarketsInfo = connect(
    mapStateToProps,
    mapDispatchToProps
)(CurrentMarketInfoContainer);
