...
export class WalletTable extends React.Component {
    constructor() {
        ...
        this.retrieveData = (list) => {
            const { fixed } = this.props.wallets.find(
                (w) => w.currency === this.props.currency
            ) || {
                fixed: 8,
            };
            ...
            return list
                .sort((a, b) => {
                    return moment(localeDate(a.created_at), "DD/MM HH:mm") >
                        moment(localeDate(b.created_at), "DD/MM HH:mm")
                        ? -1
                        : 1;
                })
                .map((item, index) => {
                    ...
                    const state =
                        "state" in item ? this.formatTxState(item.state) : "";
                    return [
        ...
        this.formatTxState = (tx) => {
            const statusMapping = {
                ...
                submitted: this.props.intl.formatMessage({
                    id: "page.body.wallets.table.pending",
                }),
                skipped: <SucceedIcon />,
            };
    ...
    render() {
        ...
        return (
            <div className="parent-history-elem__wallet">
                <div className="parent-history-elem__label">
                    {this.props.intl.formatMessage({
