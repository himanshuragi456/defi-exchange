...
import {
    resendCode, selectCodeSent,
    selectVerifyPhoneSuccess,
    sendCode, sendCodeData, sendCodeError,
    verifyPhone,
...
class PhoneComponent extends React.Component {
    constructor(props) {
                    ...
                    case "confirmationCode":
                        this.setState({
                            confirmationCodeFocused:
                                !this.state.confirmationCodeFocused,
                        });
                        break;
        ...
        this.ifPhoneNumberExists = (number) => {
            if (
                _.find(this.props.userData.phones, {
                    number: number,
                })
            ) {
                this.props.phoneExists();
            } else if(this.props.codeSent) {
        ...
        this.inputPhoneNumber = (e) => {
            const convertedText = e.trim(); // const convertedText = e.target.value.trim();
        ...
        const defaultCountryCode =
            this.getPhoneNumberFromProps().country || "US";
        this.state = {
    ...
    render() {
        ...
        return (
            <div className="form-step form-step1 col-md-6 container card">
                <div className="card-innr no-border-radius">
                    <div className="step-head">
                        <div className="step-number text-white">01</div>
                    ...
                    <div className="row">
                        <div className="col-12 col-md-6">
                            <label className="input-item-label">
                                1.{" "}
                                {this.translate(
                                    "page.body.kyc.phone.enterPhone"
                                )}
                            </label>
                            <div className="">
                                <div className="copy-wrap mgb-0-5x">
                                    ...
                                    >
                                        {this.props.codeSent ? (
                                            <em className="fas fa-redo" />
                                        ) : (
                                            <em className="fab fa-telegram-plane" />
                                        )}
                                    </button>
                                </div>
                            </div>
                        ...
                        {verifyPhoneSuccess && (
                            <p className="parent-confirm__success">
                                {verifyPhoneSuccess}
                            </p>
                        )}
                        <div className="px-2">
...
const mapStateToProps = (state) => ({
    ...
    codeSent: selectCodeSent(state),
});

const mapDispatchProps = (dispatch) => ({
    resendCode: (phone) => dispatch(resendCode(phone)),
    sendCode: (phone) => dispatch(sendCode(phone)),
    phoneExists: () => dispatch(sendCodeData()),
    phoneChanged: () => dispatch(sendCodeError()),
