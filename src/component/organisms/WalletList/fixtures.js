...
const randomInteger = (min, max) =>
    Math.floor(Math.random() * (max - min) + min); // tslint:disable-next-line:no-magic-numbers

const randomOneSomething = () => Math.floor(Math.random() * 1000) / 1000; // tslint:disable-next-line:no-magic-numbers
...
export const walletItems = (count) =>
    Array(count)
        .fill(0)
        .map((_, i) => ({
            active: false,
