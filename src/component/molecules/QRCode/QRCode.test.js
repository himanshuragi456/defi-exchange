...
describe("QRCode", () => {
    describe("#render", () => {
        ...
        it("renders correctly data with empty spaces", () => {
            ...
            const wrapper = mount(<QRCode data={address} dimensions="sm" />);
            const { src } = wrapper.find("img").first().props();
            expect(src).toContain(encodeURI(address));
        });
        it("renders correctly", () => {
            const tree = renderer
                .create(<QRCode data="sevaerverv343" dimensions="sm" />)
                .toJSON();
