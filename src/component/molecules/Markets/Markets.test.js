...
describe("Markets", () => {
    ...
    it("should correctly filter rows", () => {
        expect(
            wrapper.instance().searchFilter(["ETH / BTC", "0.123"], "btc")
        ).toBeTruthy();
        expect(
            wrapper.instance().searchFilter(["ETH / BTC", "0.342"], "ltc")
        ).toBeFalsy();
    });
    it("should set filtered data to state", () => {
        const component = mount(<Markets data={data} onSelect={onSelect} />);
        const filteredData = [["ETH / BTC", "0.123"]];
