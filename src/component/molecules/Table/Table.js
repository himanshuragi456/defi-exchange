...
class Table extends React.Component {
    ...
    renderBody(rows) {
        ...
        const rowElements = dataToBeMapped.map((r, i) => {
            const rowClass = r[0]; // const bgColor = rowClass === 'recent-trades-positive' ? 'var(--recent-trades-positive-bg-color)' : rowClass === 'recent-trades-negative' ? 'var(--recent-trades-negative-bg-color)' : '';

            const bgColor = (value) => {
                switch (value) {
                    case "recent-trades-positive":
                        return "var(--recent-trades-positive-bg-color)";

                    case "recent-trades-negative":
                        return "var(--recent-trades-negative-bg-color)";
            ...
            return (
                <tr
                    className={`base-table__row ${rowClassName(i)}`}
                    key={i}
                    style={{
                        backgroundColor: bgColor(rowClass),
                    }}
