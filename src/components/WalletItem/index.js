...
const renderLocked = (fixed, lockedAmount) => {
    return lockedAmount ? (
        <div className="base-wallet-item__amount-locked">
            <LockIcon />{" "}
            <Decimal fixed={fixed}>{lockedAmount.toString()}</Decimal>
        </div>
    ) : (
        ""
    );
...
const WalletItem = (props) => {
    ...
    const cName = `base-wallet-item pl-4 pr-3 ${
        active ? "base-wallet-item--active" : ""
    }`;
    return (
        <div style={style} className={cName}>
            <div className="base-wallet-item__info">
                <img
                    className="base-wallet-item__icon"
                    src={icon}
                    currency={currency}
                />
                <div className="base-wallet-item__description">
                    <span className="text-uppercase">{currency}</span>
                    <p className="base-wallet-currency-name text-muted-2">
                        {name}
                    </p>
                </div>
            </div>
            <div className="d-flex justify-content-center align-items-end flex-column">
                <span className="base-wallet-item__balance font-weight-bold">
                    <Decimal fixed={fixed}>
                        {balance ? balance.toString() : "0.0"}
                    </Decimal>
                </span>
                <p className="text-muted-2">{"$" + estimatedValue}</p>
            </div>
        </div>
    );
};
