import { Table } from "@components/components";
import classnames from "classnames";
import * as React from "react";

class History extends React.PureComponent {
    constructor() {
        super(...arguments);
        this.defaultHeaders = ["Time", "Action", "Price", "Amount", "Total"];
        this.title = "Trades History";

        this.mapRows = (cell, index) => {
            const { headers = this.defaultHeaders } = this.props;
            const index2 = headers.findIndex(function (header) {
                return header === "Action";
            });

            if (index === index2) {
                return this.renderAction(cell);
            } else {
                return cell;
            }
        };
    }

    render() {
        const { headers = this.defaultHeaders } = this.props;
        const tableData = this.props.data.map((row) => row.map(this.mapRows));
        return <Table data={tableData} header={headers} />;
    }

    renderAction(actionType) {
        const action = actionType ? actionType.toLowerCase() : actionType;
        const className = classnames("base-history-action", {
            "base-history-action--buy": action === "buy",
            "base-history-action--sell": action === "sell",
        });
        return <span className={className}>{action}</span>;
    }
}

export { History };
