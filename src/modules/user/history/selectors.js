...
export const selectPageCount = (state, limit) =>
    Math.ceil(state.user.history.fullHistory / limit);
export const selectFirstElemIndex = (state, limit) =>
    state.user.history.page * limit + 1;
export const selectLastElemIndex = (state, limit) => {
    if (!selectFullHistory(state)) return state.user.history.list.length;
...
export const selectNextPageExists = (state, limit) =>
