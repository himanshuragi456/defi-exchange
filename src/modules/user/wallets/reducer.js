...
const hideWallet =
    localStorage.getItem("hideEmptyWallets") &&
    localStorage.getItem("hideEmptyWallets") === "true";
...
const walletsListReducer = (state, action) => {
    switch (action.type) {
        ...
        case WALLETS_DATA: {
            ...
            const btcIndex = list.findIndex(
                (wallet) => wallet.currency.toLowerCase() === "btc"
            );
            if (btcIndex >= 0) {
        ...
        case HIDE_EMPTY_WALLETS:
            return {
                ...state,
                hideEmptyWallets: action.payload.hideEmptyWallet,
            };
        default:
            return state;
    }
