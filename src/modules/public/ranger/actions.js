    ...
    if (incrementalOrderBook()) {
        store.dispatch(
            depthIncrementSubscribe({
                market_id: market.id,
                matching_engine: market.matching_engine,
            })
        );
...
export const rangerSubscribeMarket = (market) =>
    rangerSubscribe(marketStreams(market));
export const rangerUnsubscribeMarket = (market) =>
    rangerUnsubscribe(marketStreams(market));
export const rangerSubscribeKlineMarket = (marketId, periodString) =>
    rangerSubscribe(marketKlineStreams(marketId, periodString));
export const rangerUnsubscribeKlineMarket = (marketId, periodString) =>
