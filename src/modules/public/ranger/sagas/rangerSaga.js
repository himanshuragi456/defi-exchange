...
import { walletBalanceData, walletsAddressData } from "../../../user/wallets";
import {userOrdersHistoryAppendData} from "../../../user/ordersHistory";
        ...
        ws.onmessage = ({ data }) => {
            ...
            for (const routingKey in payload) {
                if (payload.hasOwnProperty(routingKey)) {
                    ...
                    const orderBookMatchSnap =
                        routingKey.match(/([^.]*)\.ob-snap/);
                    const orderBookMatchInc =
                        routingKey.match(/([^.]*)\.ob-inc/);

                    // public
                    if (orderBookMatch) {
                        if (
                            currentMarket &&
                            orderBookMatch[1] === currentMarket.id
                        ) {
                    ...
                    if (orderBookMatchSnap) {
                        if (
                            currentMarket &&
                            orderBookMatchSnap[1] === currentMarket.id
                        ) {
                    ...
                    if (orderBookMatchInc) {
                        if (
                            currentMarket &&
                            orderBookMatchInc[1] === currentMarket.id
                        ) {
                            ...
                            if (previousSequence + 1 !== event.sequence) {
                                //TODO: Check the ranger disconnect when orderbook's sequence are bad
                                window.console.log(
                    ...
                    // public
                    const tradesMatch =
                        String(routingKey).match(/([^.]*)\.trades/);
                    if (tradesMatch) {
                        emitter(
                            recentTradesPush({
                                trades: event.trades,
                    ...
                    switch (routingKey) {
                        ...
                        case "trade":
                            ...
                            return;
                        case "account":
                            emitter(walletBalanceData(event));
                            return;
                        case "deposit_address":
                            emitter(walletsAddressData(event));
                            return;
                        default:
                    }
                    window.console.log(
...
function* dispatchCurrentMarketOrderUpdates(action) {
    ...
    yield put(userOrdersHistoryAppendData(action.payload))
}
...
export function* rangerSagas() {
    ...
    yield takeEvery(
        RANGER_USER_ORDER_UPDATE,
        dispatchCurrentMarketOrderUpdates
    );
    while (true) {
