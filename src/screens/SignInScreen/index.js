...
import { selectTempEmail } from "../../modules/user/auth";
...
class SignIn extends React.Component {
    constructor() {
        ...
        this.renderSignInForm = () => {
            ...
            return (
                <SignInComponent
                    ...
                    message2fa={this.props.intl.formatMessage({
                    ...
                    handleChange2faFocusField={this.handle2faFocus}
                    label2fa={this.props.intl.formatMessage({
                        id: "page.body.wallets.tabs.withdraw.content.code2fa",
                    })}
                    signUpText={this.props.intl.formatMessage({
                        id: "page.signIn.signup",
                    })}
                    signUpExists={this.props.intl.formatMessage({
                        id: "page.signIn.signupExists",
                    })}
                    forgetPasswordText={this.props.intl.formatMessage({
                        id: "page.body.forget_password",
                    })}
                    recaptchaOnChange={this.onCaptchaChange}
                    hasConfirmed={recaptchaConfirmed}
                    recaptchaConfirmed={recaptchaConfirmed}
                    exchangeSignInText={this.props.intl.formatMessage(
                        {
                            id: "page.signIn.description",
                        },
                        {
                            exchange_name: exchangeName,
                        }
                    )}
                    require2fa={this.props.require2FA}
                    welcomeLabel={this.props.intl.formatMessage({
                        id: "page.sigin.welcomeText",
                    })}
                />
            );
        };
        ...
        this.handleSignIn = () => {
            ...
            if (captchaType() === "recaptcha" && recaptchaConfirmed !== true)
                return;
            this.props.signIn({
                email,
                password,
        ...
        this.handle2FASignIn = () => {
            ...
            if (!otpCode) {
                this.setState({
                    error2fa: this.props.intl.formatMessage({
                        id: "page.signIn.2faRequired",
                    }),
                });
            } else {
                this.props.signIn({
    ...
    componentDidMount() {
        if (
            ...
            this.props.userData.level === 0
        ) {
            this.props.history.push("/email-verification", {
                email: this.props.tempEmail,
            });
            return;
        }
    ...
    componentWillReceiveProps(props) {
        ...
        if (props.requireEmailVerification || props.userData.level === 0) {
    ...
    render() {
        ...
        const className = cx("parent-sign-in-screen__container", {
            loading,
        });
        return (
            <div className="page-ath-wrap pt-5 pb-5 d-flex justify-content-start align-items-center flex-column">
                {/*<AuthBrand />*/}
                <div
...
const mapStateToProps = (state) => ({
    ...
    tempEmail: selectTempEmail(state),
});
...
const mapDispatchProps = (dispatch) => ({
    ...
    signUpRequireVerification: (data) =>
        dispatch(signUpRequireVerification(data)),
});

const SignInScreen = injectIntl(
    withRouter(connect(mapStateToProps, mapDispatchProps)(SignIn))
);
export { SignInScreen };
