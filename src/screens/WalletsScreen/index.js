...
class WalletsComponent extends React.Component {
    constructor(props) {
        ...
        this.isOtpDisabled = () => {
            return (
                <div className="text-cengter1 pt-5">
        ...
        this.toggleProofModal = () => {
            this.setState({proofModal: !this.state.proofModal})
        }
    ...
    componentWillUnmount() {
        ...
        document
            .getElementsByClassName("parent-layout")[0]
            .classList.remove("parent-layout__custom-wrapper");
    ...
    render() {
        ...
        const walletCurrencies = formattedWallets.filter((data) =>
            walletCustomCurrency().includes(data.currency)
        );
        ...
        const defaultCurrency = currencies.filter(
            (curr) =>
                curr.id.toLowerCase() === "usd" ||
                curr.id.toLowerCase() === "usdt"
        )[0];
        const defaultCurrencyWallet =
            defaultCurrency &&
            wallets.filter(
                (wallet) => wallet.currency === defaultCurrency.id
            )[0];
        const nonZeroWallets = formattedWallets.filter(
            (element) => Number(element.balance) !== 0
        );
        ...
        return (
            ...
            >
                                ...
                                return (
                                    <div className="new-card-wallet">
                                        <div className="new-card-wallet-item">
                                            ...
                                            >
                                                <h6 className="new-card-wallet-list-heading">
                                                    {e.name}
                                                </h6>
                            ...
                            >
                                ...
                                {mobileWallet ? (
                                    ...
                                    <WalletListDesktop
                                        {...{
                                            usdPrecision:
                                                usdPrecision.precision ||
                                                usdPrecision,
                                            marketTickers: marketTickers,
                                            onWalletSelectionChange:
                                                this.onWalletSelectionChange,
                                            walletItems: emptyWallets
                                                ? nonZeroWallets
                                                : filteredWallets ||
                                                  formattedWallets,
                ...
                <ProofModal currency={selectedCurrency} show={proofModal} alert={this.props.fetchSuccess} hide={this.toggleProofModal} />
            </div>
        );
    }
    ...
    renderDeposit(wallet) {
        const { addressDepositError, wallets, currencies, user } = this.props;
        ...
        if (wallet.type === "coin") {
            return (
                <React.Fragment>
                    <DepositCrypto
                        data={walletAddress}
                        handleOnCopy={[
                            "page.body.wallets.tabs.deposit.ccy.message.success",
                        ]}
                        error={error}
                        text={text}
            ...
            return (
                <React.Fragment>
                    <DepositFiat
                        currency={wallet.currency}
                        title={this.translate('page.body.wallets.tabs.deposit.fiat.message1')}
                        description={this.translate('page.body.wallets.tabs.deposit.fiat.message2')}
                        uid={user ? user.uid : ''}
                        minDepositAmount={min_deposit_amount}
                        depositEnabled={deposit_enabled}
                        depositFee={deposit_fee}
                        minDepositAmountText={this.translate('page.body.wallets.tabs.deposit.info.minDepositAmount')}
                        depositEnabledText={this.translate('page.body.wallets.tabs.deposit.info.depositEnabled')}
                        depositFeeText={this.translate('page.body.wallets.tabs.deposit.info.depositFee')}
                        disabledText={this.translate('page.body.status.disabled')}
                        enabledText={this.translate('page.body.status.enabled')}
                        referenceCodeText={this.translate('page.body.wallets.tabs.deposit.fiat.referenceCode')}
                        renderProof={this.toggleProofModal}
                    />
                    {currency && (
                        <WalletHistory
                            label="deposit"
